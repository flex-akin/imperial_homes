...
const findTenor = () => {
    ...
    const ageNew = 60 - age 
    if ( ageNew >= 20 ) {
        document.getElementById("tenor").innerText = `Tenor can't be more than 20 years`
        document.getElementById("tenure").setAttribute("max", 20)

    }
    else if ( ageNew < 20 & ageNew > 0) {
        document.getElementById("tenor").innerText = `Tenor can't be more than ${ageNew} years`
...
const PMT =( rate, nper, pv, fv, type) => {
    ...
    document.getElementById("monthlyRepaymentResult").innerText = `₦${monthlyRepayment}`
