document.addEventListener('DOMContentLoaded', function() {
  ...
  fetch('https://propertyapi.ivantage.africa/api/ivantage/properties?size=52&page=1')
    .then((response) => {
      if (!response.ok) {
    ...
    .then((jsonData) => {
      ...
      renderData(data.data.propertyData, 
        data.data.nextPage, 
    ...
    loadNextPage =(page) => {
      fetch(`https://propertyapi.ivantage.africa/api/ivantage/properties?size=52&page=${page}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
      ...
      cardBody.innerHTML = `
        <div>
          ...
          <div class="card-body p-0">
            <h5 class="card-title p-0" style="font-size: 17px;">${data[i].projectName}</h5>
          </div>
          <div class="felix">
            <i style="color : red" class="fa-solid fa-location-dot"> &nbsp; </i>${data[i].address + ", " + data[i].state}
          </div>
          <div class="d-flex justify-content-between flex-row p-0 ps-0 m-0">
            <div class="fw-bold fs-6 p-0">₦${data[i].price.toLocaleString()}</div>
            <div>
              <div class="felix">
                <i class="fa-solid fa-bed p-0"></i> ${" " + data[i].numberOfBedrooms}
              </div>
            </div>
          </div>
        </div>`;
